...
def _clear(probe) -> None:
    """Clear one probe's cache, tolerating a probe that has been replaced.

    A test that patches a probe off (`monkeypatch.setattr(tools, "bgzip",
    lambda: ...)`) leaves a plain function under that name, which has no
    `cache_clear`. Whether `reset_cache` sees the patched or the original
    function then depends purely on fixture teardown *order* -- so a test file
    that worked would start raising AttributeError because an unrelated autouse
    fixture was added elsewhere. There is nothing to forget for a probe that is
    not the cached original, so skipping it is also the correct behaviour.
    """
