...
Entries closed out from `docs/TODO.md`, kept as a record rather than
deleted (project convention: a `— FIXED` entry documents why the code
looks the way it does; a deleted one is a question someone will ask
again). Split out on 2026-08-02 so the active backlog doesn't carry every
closed entry's full context on every read.
...
## Aligners: STAR and DRAGMAP — STAR FIXED, DRAGMAP WON'T DO

STAR shipped 2026-08-01 (`Merge STAR aligner support with directory-shaped
index layout`, plus a same-day follow-up fix). DRAGMAP was tracked as
[#12](https://github.com/syntheticgio/bioflow/issues/12) and closed
2026-08-05 as won't-do: no DRAGEN-compatibility request has ever come in, and
the user decided to wait for one rather than build the tool speculatively.

**The arm64 investigation #12 asked for happened before the close, and it
overturned this entry's own premise.** This entry (below) assumed DRAGMAP was
"the same class of problem as DeepVariant" and would need a from-source
sse2neon build like bwa-mem2's. It would not have: **bioconda already
publishes a `linux-aarch64` build of `dragmap` 1.3.0** (build `_6`,
2025-08-19), with the sse2neon translation carried in bioconda's own
39 KB `0001-aarch64-arm64.patch` and `build.sh` -- maintained upstream, not by
this repo. DRAGMAP has no Debian package at all, so bioconda would have been
the only route on *either* architecture, meaning no `TARGETARCH` branch in
the Dockerfile the way bwa-mem2 and NCBI Datasets need one -- one
`install-dragmap.sh` in the shape of `install-clair3.sh` would have covered
both arches identically.

**The real risk the investigation found was the opposite one: AVX2 on x86,
not arm64.** Bioconda's `linux-64` build compiles with `-march=x86-64-v3`,
which mandates AVX2 -- a CPU without it gets `SIGILL`, not a clean
"unsupported" message. The arm64 build (`-march=armv8-a`, baseline NEON) is
actually the more portable of the two. Had this shipped, the tool probe would
have needed to check for AVX2 and mark the card unavailable with a reason,
the same pattern `TOOL_META`'s `unavailable` reasons already use elsewhere,
rather than let a run die mid-alignment.

**Two more open questions the investigation flagged but did not resolve**,
since there was no implementation to resolve them against: bioconda's most
recent build (`_7`, 2025-09-30) dropped from 8.4 MB to 0.5 MB and lost its
`boost` runtime dependency, which is either a real slimming or a broken
build, and whichever it is decides which build number gets pinned; and
`dragen-os --version` was never confirmed to exist -- bioconda's own package
test invokes `-h` instead, and STAR's version-string truncation
(`2.7.11b` -> `2.7.11`) is the standing reminder that a probe flag should be
run against the real binary rather than assumed from the tool's docs.

If a DRAGEN-compatibility request does arrive, re-open against this evidence
rather than re-running the arm64 investigation from scratch: the viability
question is answered (yes, via bioconda, on both arches), and what remains is
the AVX2 probe, a measured `MemoryModel` (DRAGEN hash tables are the same
class of "guess is wrong by 4x" risk STAR's was), a `suggestion_service`
decision (STAR's precedent below argues for none, deliberately, with a test
asserting bwa-mem2 stays preferred), and `TOOL_META` -- license is
GPL-3.0-or-later per the recipe but should be re-verified against
`Illumina/DRAGMAP`'s own `COPYRIGHT` before being written down, and there is
no DRAGMAP paper, so `citation` is honestly empty or points at the DRAGEN
methods paper.

---

The rest of this entry is STAR's, left as originally written.
